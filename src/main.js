...
const D={dias:14,sem:3,mes:12,medio:1,aviso:7,semana:7,modo:'mes',ref:'',marks:'42d, 2m, 4m, 6m, 9m, 12m, 15m, 18m, 24m',
  nodo:'',supervisora:'',
  modulos:'Módulo A|6|Motor grueso,Lenguaje,Cognitivo,Social y emocional; Módulo B|12|Motor grueso,Motor fino,Lenguaje,Cognitivo; Módulo C|24|Motor grueso,Motor fino,Lenguaje,Cognitivo,Social y emocional; Módulo D|48|Motor grueso,Motor fino,Lenguaje,Cognitivo,Social y emocional'};
const AREAS=['Motor grueso','Motor fino','Lenguaje','Cognitivo','Social y emocional'];
const areaCls=a=>/fino/i.test(a)?'a-mf':/motor/i.test(a)?'a-mg':/lenguaje/i.test(a)?'a-le':/social/i.test(a)?'a-se':'a-co';
let S={kids:[],sel:null,cfgOpen:false,ask:false,msg:'',user:null,ready:false,err:'',em:'',openCtl:null};
let cfg={...D};
...
function parseModulos(){return cfg.modulos.split(';').map(s=>{const [nombre,hasta,areas]=s.split('|');
  return nombre?{nombre:nombre.trim(),hasta:+((hasta||'').trim())||999,areas:(areas||'').split(',').map(a=>a.trim()).filter(Boolean)}:null}).filter(Boolean).sort((a,b)=>a.hasta-b.hasta)}
function moduloFor(tm){const ms=parseModulos();return ms.find(m=>tm<=m.hasta)||ms[ms.length-1]||null}
function areasFor(tm){const m=moduloFor(tm);return (m&&m.areas.length)?m.areas:AREAS}
function kidModulo(k){const i=info(k);return i&&!i.bad?((moduloFor(i.tm)||{}).nombre||''):''}
function nextVisitDate(bs){if(!bs)return '';const b=pd(bs),r=pd(iso(now()));if(dd(b.date,r.date)<0)return '';
  const x=diff(b,r);let n=x.y*12+x.m,d=addMonths(b,n);if(dd(r.date,d)<0||n===0){n=n+1;d=addMonths(b,n)}return iso(d)}
function periodLogs(k,ctlList,idx){const lo=idx===0?null:ctlList[idx-1].dt,hi=ctlList[idx].dt;
  return (k.log||[]).map((e,ix)=>({...e,ix})).filter(e=>{const d=pd(e.d).date;return (lo===null||d>lo)&&d<=hi})}
function ctlPanelHtml(plogs,allowed,key){
  const list=plogs.length?plogs.map(e=>`<div class="plog"><span><b>${esc(e.a)}</b> — ${esc(e.n)} <span style="color:var(--mute)">(${fd(pd(e.d).date)})</span></span><button class="sec" data-act="dellog" data-ix="${e.ix}" aria-label="Eliminar seguimiento">Quitar</button></div>`).join(''):'<p class="note" style="margin:0">Aún no hay seguimiento registrado para este período.</p>';
  return `<div class="ctl-panel">${list}
  <div class="g2"><div><label>Fecha</label><input type="date" id="ld2" value="${esc(key)}"></div>
  <div><label>Área</label><select id="la2">${allowed.map(a=>`<option>${a}</option>`).join('')}</select></div></div>
  <div><label>Actividad y observaciones</label><textarea id="ln2" rows="2" placeholder="Qué se trabajó, cómo respondió"></textarea></div>
  <div class="btns"><button data-act="addlogp" data-key="${key}">Guardar</button></div></div>`}
const mlab=n=>{const y=Math.floor(n/12),m=n%12;return pl(n,'mes','meses')+(n>=12?' ('+pl(y,'año','años')+(m?', '+pl(m,'mes','meses'):'')+')':'')};
function ctl(i){if(cfg.modo==='lista')return marks().map(m=>({dt:mDate(i.b,m),lb:mLabel(m)}));const o=[];for(let n=1;n<=48;n++)o.push({dt:addMonths(i.b,n),lb:mlab(n)});return o}
...
function exportAll(){
  const head=['N.º de Cupo','Nombre del Niño','Fecha de Nacimiento','Fecha de visita','Meses cumplidos','Módulo','Semana','Días Cumplidos','Hora de visita'];
  const rows=S.kids.map(k=>{const vi=k.visit&&k.bs?info(k,k.visit):null;
    const meses=vi&&!vi.bad?vi.tm:'';
    const dias=vi&&!vi.bad?vi.days:'';
    const semana=vi&&!vi.bad?Math.floor(vi.days/7):'';
    const mod=vi&&!vi.bad?((moduloFor(vi.tm)||{}).nombre||''):kidModulo(k);
    return [k.cupo,k.nm,k.bs,k.visit,meses,mod,semana,dias,k.hv]});
  const lines=[['NODO:',cfg.nodo||''],['SUPERVISORA:',cfg.supervisora||''],[],head,...rows];
  const csv=lines.map(r=>r.map(csvCell).join(';')).join('\n');
  const blob=new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8;'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');
  a.href=url;a.download='tabla-registro-visitas-'+iso(now())+'.csv';
...
<div><label>Nodo</label><input type="text" data-c="nodo" value="${esc(cfg.nodo)}"></div>
<div><label>Supervisora</label><input type="text" data-c="supervisora" value="${esc(cfg.supervisora)}"></div></div>
<div style="margin-top:12px"><label>Módulos por edad y áreas que trabaja cada uno</label><input type="text" data-c="modulos" value="${esc(cfg.modulos)}"><div class="hint">Formato: Nombre|hasta_meses|áreas separadas por coma; siguiente módulo... El módulo y las áreas de cada niño se asignan automáticamente según su edad en meses.</div></div>
...
<div><label>Fechas de control</label><select data-c="modo"><option value="mes"${cfg.modo==='mes'?' selected':''}>Cada cumple mes (hasta 48 meses)</option><option value="lista"${cfg.modo==='lista'?' selected':''}>Lista personalizada</option></select><div class="hint">Cada cumple mes usa el día de nacimiento en cada mes.</div></div>
...
const wd=(n,s,p)=>n===1?s:p;
function iconStat(name){const icons={kids:'<circle cx="12" cy="8" r="4"/><path d="M4 21c0-4 4-6 8-6s8 2 8 6"/>',
  warn:'<rect x="3" y="5" width="18" height="16" rx="2"/><path d="M8 3v4M16 3v4M3 10h18"/>',
  late:'<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/>',
  ok:'<path d="M20 6 9 17l-5-5"/>'};
  return `<svg class="ic" viewBox="0 0 24 24">${icons[name]||''}</svg>`}
function summaryHtml(){const soon=S.kids.filter(k=>k.visit&&visit(k).c==='warn').length;
  const late=S.kids.filter(k=>k.visit&&visit(k).c==='late').length;
  const okPct=S.kids.length?Math.round(100*(S.kids.length-late)/S.kids.length):100;
  return `<div class="stats">
  <div class="stat"><div class="bub">${iconStat('kids')}</div><b>${S.kids.length}</b><span>${wd(S.kids.length,'niño','niños')}</span></div>
  <div class="stat"><div class="bub">${iconStat('warn')}</div><b>${soon}</b><span>próximas</span></div>
  <div class="stat"><div class="bub">${iconStat('late')}</div><b>${late}</b><span>${wd(late,'vencida','vencidas')}</span></div>
  <div class="stat"><div class="bub">${iconStat('ok')}</div><b>${okPct}%</b><span>al día</span></div>
  </div>`}
function listHtml(){if(!S.kids.length)return summaryHtml()+'<div class="card empty">Aún no hay niños. Usa «Agregar niño» para empezar.</div>';
  const ks=[...S.kids].sort((a,b)=>(a.visit||'9999')<(b.visit||'9999')?-1:1);
  return summaryHtml()+'<h2>🧒 Agenda de visitas</h2>'+ks.map(k=>{const i=info(k),v=visit(k),mo=kidModulo(k);
    return `<button class="kid" data-act="sel" data-id="${k.id}" data-sx="${k.sx||''}" data-ini="${esc((k.nm||'?').trim().charAt(0).toUpperCase())}"><b>${k.cupo?'#'+esc(k.cupo)+' · ':''}${esc(k.nm||'Sin nombre')}</b><span class="a">${sexo[k.sx||'']}${mo?' · '+esc(mo):''} · ${i&&!i.bad?i.cl:'Falta fecha de nacimiento'}</span><span class="chip ${v.c}">${v.t}</span></button>`}).join('')+weekHtml()}

function detailHtml(k){const i=info(k),v=visit(k),vi=k.visit&&k.bs?info(k,k.visit):null,mo=kidModulo(k);
  ...
  <div><label>Módulo</label><input value="${esc(mo)}" disabled><div class="hint">${mo?'Asignado automáticamente según la edad.':'Se asigna al ingresar la fecha de nacimiento.'} Ajusta los rangos en Configuración.</div></div>
  <div><label>Fecha de nacimiento</label><input type="date" data-f="bs" value="${esc(k.bs)}"></div>
  <div><label>Fecha de visita</label><input type="date" data-f="visit" value="${esc(k.visit)}"><div class="hint">${k.visit?v.t+' · '+fd(pd(k.visit).date):''}${vi&&!vi.bad?'<br>Edad ese día: '+vi.cl+' ('+vi.ex+')':''}${k.bs?'<br>Calculada según el cumple mes; puedes editarla.':''}</div></div>
  ...
  if(i&&i.bad)body='<div class="card empty">La fecha de nacimiento es posterior a la fecha de cálculo.</div>';
    ...
    const ctlList=ctl(i);
    const allowedNow=areasFor(i.tm);
    const items=ctlList.map((m,idx)=>{const dt=m.dt,l=dd(i.r.date,dt);let c='',s;
      if(l<0){c='done';s='Cumplido'}else{if(!nx){c='next';nx=true}s=l===0?'Hoy':l===1?'Mañana':'En '+pl(l,'día','días')}
      const plogs=periodLogs(k,ctlList,idx),key=iso(dt),open=S.openCtl===key;
      const doneChip=plogs.length?'<span class="s-ok">✓ Registrado</span>':'';
      return `<li class="${c}${open?' open':''} ctl"><button class="ctl-row" data-act="togglectl" data-key="${key}"><span class="t">${m.lb}</span><span class="d">${fd(dt)}</span>${doneChip}<span class="s">${s}</span></button>${open?ctlPanelHtml(plogs,allowedNow,key):''}</li>`}).join('');
    const wk=Math.floor(i.days/7),wd2=i.days%7;
    ...
    <div class="stat"><b>${wk}</b><span>semanas${wd2?' + '+pl(wd2,'día','días'):''}</span></div>
    ...
    <section class="card"><h2>🩺 Fechas de control · hasta 48 meses</h2><p class="note" style="margin:0 0 10px">Toca una fecha para ver o registrar el seguimiento de ese período.</p><ul class="tl">${items||'<li class="d">Sin fechas configuradas.</li>'}</ul></section>`}
  ...
  const allowed=i&&!i.bad?areasFor(i.tm):null;
  const planItems=pn&&allowed?pn[2].filter(a=>allowed.some(ar=>areaCls(ar)===areaCls(a[0]))):(pn?pn[2]:[]);
  const planShown=planItems.length?planItems:(pn?pn[2]:[]);
  const plan=pn?`<section class="card"><h2>🎯 Plan de actividades · ${pn[1]}${mo?' · '+esc(mo):''}</h2><ul class="tl">${planShown.map(a=>`<li class="${areaCls(a[0])}"><span class="area-tag">${esc(a[0])}</span><span class="t">${a[1]}</span></li>`).join('')}</ul><p class="note">Ideas generales de juego. Adáptalas al ritmo de cada niño y consulta si notas retrasos.</p></section>`:'';
  const areaOpts=(allowed||AREAS).map(a=>`<option>${a}</option>`).join('');
  const est=`<section class="card"><h2>📝 Seguimiento adicional</h2>
  <p class="note" style="margin:0 0 10px">Para sesiones especiales fuera de una fecha de control. ${log.length?pl(log.length,'sesión registrada en total','sesiones registradas en total')+(last!=null?' · última hace '+pl(last,'día','días'):''):'Aún no hay sesiones registradas.'}</p>
  <div class="g2"><div><label>Fecha</label><input type="date" id="ld" value="${iso(now())}"></div>
  <div><label>Área</label><select id="la">${areaOpts}</select></div></div>
  ...
  <ul class="tl" style="margin-top:12px">${log.map(e=>`<li class="${areaCls(e.a)}"><span class="area-tag">${esc(e.a)}</span><span class="d">${fd(pd(e.d).date)} — ${esc(e.n)}</span><button class="sec" data-act="dellog" data-ix="${e.ix}" aria-label="Eliminar sesión">Quitar</button></li>`).join('')}</ul></section>`;
  ...
  return `<div class="btns" style="margin-bottom:14px"><button class="sec" data-act="back">← Todos los niños</button></div>${form}${body}${plan}${est}<div class="btns">${del}</div>`}
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-act]');if(!b)return;const a=b.dataset.act,k=S.kids.find(x=>x.id===S.sel);S.msg='';S.err='';
  ...
  if(!S.user)return;
  if(a==='add'){const n={id:crypto.randomUUID(),cupo:'',nm:'',sx:'',modulo:'',bs:'',visit:'',hv:'',log:[]};S.kids.push(n);S.sel=n.id;S.ask=false;S.openCtl=null;saveKid(n)}
  else if(a==='sel'){S.sel=b.dataset.id;S.ask=false;S.openCtl=null}
  else if(a==='back'){S.sel=null;S.ask=false;S.openCtl=null}
  ...
  else if(a==='togglectl'){S.openCtl=S.openCtl===b.dataset.key?null:b.dataset.key}
  else if(a==='addlogp'&&k){const n=$('ln2').value.trim(),d=$('ld2').value||b.dataset.key;if(n&&d){const e={id:crypto.randomUUID(),d,a:$('la2').value,n};(k.log=k.log||[]).push(e);sb.from('sesiones').insert({id:e.id,nino_id:k.id,fecha:d,area:e.a,nota:n}).then(chk);S.msg='Seguimiento guardado.'}}
...
document.addEventListener('change',e=>{const t=e.target;
  if(t.dataset.f){const k=S.kids.find(x=>x.id===S.sel);if(k){k[t.dataset.f]=t.value;
    if(t.dataset.f==='bs'){k.visit=nextVisitDate(k.bs);k.modulo=kidModulo(k)}
    S.ask=false;saveKid(k);render()}}
  else if(t.dataset.c){const c=t.dataset.c;cfg[c]=(c==='marks'||c==='ref'||c==='modo'||c==='nodo'||c==='supervisora'||c==='modulos')?t.value:Math.max(0,parseInt(t.value,10)||0);saveCfg();render()}});
render();
