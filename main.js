...
function exportAll(){
  const head=['Cupo','Nombre','Sexo','Módulo','Fecha nacimiento','Edad actual','Próxima visita','Hora visita'];
  const rows=S.kids.map(k=>{const i=info(k);return [k.cupo,k.nm,sexo[k.sx||''],k.modulo,k.bs,i&&!i.bad?i.cl:'',k.visit,k.hv]});
  const csv=[head,...rows].map(r=>r.map(csvCell).join(';')).join('\n');
  const blob=new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8;'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');
  a.href=url;a.download='registro-ninos-'+iso(now())+'.csv';
...
function summaryHtml(){const soon=S.kids.filter(k=>k.visit&&visit(k).c==='warn').length;
  ...
  <div class="stat"><div class="bub">${iconStat('kids')}</div><b>${S.kids.length}</b><span>${pl(S.kids.length,'niño','niños')}</span></div>
  <div class="stat"><div class="bub">${iconStat('warn')}</div><b>${soon}</b><span>próximas</span></div>
  <div class="stat"><div class="bub">${iconStat('late')}</div><b>${late}</b><span>${pl(late,'vencida','vencidas')}</span></div>
...
function listHtml(){if(!S.kids.length)return summaryHtml()+'<div class="card empty">Aún no hay niños. Usa «Agregar niño» para empezar.</div>';
  ...
  return summaryHtml()+'<h2>🧒 Agenda de visitas</h2>'+ks.map(k=>{const i=info(k),v=visit(k);
    return `<button class="kid" data-act="sel" data-id="${k.id}" data-sx="${k.sx||''}" data-ini="${esc((k.nm||'?').trim().charAt(0).toUpperCase())}"><b>${k.cupo?'#'+esc(k.cupo)+' · ':''}${esc(k.nm||'Sin nombre')}</b><span class="a">${sexo[k.sx||'']}${k.modulo?' · '+esc(k.modulo):''} · ${i&&!i.bad?i.cl:'Falta fecha de nacimiento'}</span><span class="chip ${v.c}">${v.t}</span></button>`}).join('')+weekHtml()}
